...
![1770785548619](image/README/1770785548619.png)
...
| ------------------- | ------------------------------------------------------------ | ------------------------------------------ |
| `GEMINI_API_KEY`  | Google AI Studio에서 발급받은 API 키<br />(0단계 3번 참조)   | AIzaSy~                                    |
| `EMAIL_USER`      | 알림을 보낼 Gmail 계정 주소<br />(0단계 2번 참조)            | sample3@example.com                        |
| `EMAIL_PASSWORD`  | 이메일 발송용 계정의 Gmail 앱 비밀번호<br />(0단계 2번 참조) | aaaa bbbb cccc dddd                        |
| `EMAIL_RECEIVER`  | 뉴스를 받을 수신자 이메일<br />(쉼표로 여러 명 가능)         | sample@example.com, sample2@example.com, ~ |
...
1. `config.json` 파일을 수정하여 원하는 정보를 골라받으세요.
   ...
   ![1770775973663](image/README/1770775973663.png)
2. 연필모양 버튼 클릭
